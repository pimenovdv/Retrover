...
async def test_opacity_control(test_server):
    async with async_playwright() as p:
        ...
        await page.evaluate("document.getElementById('properties-panel').style.display = 'block';")

        # Check that properties panel is visible
        is_panel_visible = await page.evaluate("document.getElementById('properties-panel').style.display !== 'none'")
        assert is_panel_visible, "Properties panel should be visible when an object is selected"
        ...
        assert initial_opacity == 1.0 or initial_opacity is None, f"Initial opacity should be 1.0 or undefined, got {initial_opacity}"
