    ...
    with TestClient(app) as client:
        with client.websocket_connect("/ws/default/testuser1") as websocket1:
            with client.websocket_connect("/ws/default/testuser2") as websocket2:
                ...
                websocket1.send_text(
                    json.dumps({"action": "cursor", "object": {"x": 100, "y": 200}})
                )
                ...
                websocket1.send_text(
                    json.dumps({"action": "select", "object": {"id": "123"}})
                )
                ...
                websocket1.send_text(
                    json.dumps({"action": "chat", "object": {"message": "Hello!"}})
                )
...
async def test_batch_updates():
    ...
    await db_batcher.push("remove", {"id": "test_shape_1"})
    ...
    with TestClient(app) as client:
        with client.websocket_connect("/ws/board1/user1") as ws1:
            with client.websocket_connect("/ws/board2/user2") as ws2:
                ...
                ws1.send_text(
                    json.dumps(
                        {"action": "chat", "object": {"message": "Hello board1!"}}
                    )
                )
                ...
                with client.websocket_connect("/ws/board1/user3") as ws3:
                    ...
                    ws3.send_text(
                        json.dumps(
                            {"action": "chat", "object": {"message": "Hi user1"}}
                        )
                    )
...
async def test_shape_properties_persistence():
    ...
    async with AsyncSessionLocal() as session:
        result = await session.execute(
            select(Shape).filter(Shape.id == "test_shape_props")
        )
        shape = result.scalars().first()
        assert shape is not None
        assert shape.type == "rect"
    ...
    # Modify properties
    await db_batcher.push(
        "modify", {"id": "test_shape_props", "stroke": "blue", "strokeWidth": 10}
    )
...
async def test_websocket_actions(setup_db_sync):
    ...
    with TestClient(app) as client:
        with client.websocket_connect("/ws/default/ws_user") as websocket:
            ...
            websocket.send_text(
                json.dumps({"action": "deselect", "object": {"id": "123"}})
            )
        ...
        with client.websocket_connect("/ws/default/ws_user1") as ws1:
            with client.websocket_connect("/ws/default/ws_user2") as ws2:
                ...
                ws1.send_text(
                    json.dumps({"action": "cursor", "object": {"x": 10, "y": 20}})
                )
...
async def test_db_worker_exception(setup_db_sync):
    ...
    from src.main import db_batcher, db_writer_worker
    ...
    with TestClient(app) as client:
        ...
        with open(test_file_path, "rb") as f:
            response = client.post(
                "/upload", files={"file": ("test_img.png", f, "image/png")}
            )
...
async def test_upload_invalid_type_endpoint():
    ...
    with TestClient(app) as client:
        ...
        with open(test_file_path, "rb") as f:
            response = client.post(
                "/upload", files={"file": ("test_doc.txt", f, "text/plain")}
            )
...
async def test_upload_pdf_endpoint():
    ...
    with TestClient(app) as client:
        ...
        with open(test_file_path, "rb") as f:
            response = client.post(
                "/upload", files={"file": ("test_doc.pdf", f, "application/pdf")}
            )
            assert response.status_code == 200
            data = response.json()
            assert "urls" in data
...
async def test_upload_corrupted_pdf_endpoint():
    ...
    with TestClient(app) as client:
        ...
        with open(test_file_path, "rb") as f:
            response = client.post(
                "/upload", files={"file": ("corrupted_doc.pdf", f, "application/pdf")}
            )
            assert response.status_code == 400
...
async def test_upload_invalid_extension():
    ...
    with TestClient(app) as client:
        ...
        with open(test_file_path, "rb") as f:
            response = client.post(
                "/upload", files={"file": ("test_doc.xyz", f, "image/png")}
            )
            assert response.status_code == 400
    ...
    with patch("src.main.decode_access_token", return_value={"sub": "nick"}):
        try:
            ...
            await websocket_endpoint(
                mock_ws, "test_board_err", "nick", "fake_token", mock_db
            )
        except Exception:
            pass
