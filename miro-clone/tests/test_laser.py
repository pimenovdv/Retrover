...
async def test_laser_pointer():
    ...
    try:
        async with async_playwright() as p:
            ...
            initial_circles_count = await page.evaluate(
                'canvas.getObjects().filter(o => o.type === "circle" && o.is_background).length'
            )
            ...
            new_circles_count = await page.evaluate(
                'canvas.getObjects().filter(o => o.type === "circle" && o.is_background).length'
            )
            assert new_circles_count > initial_circles_count

            # Verify they have is_background: true
            has_bg_true = await page.evaluate(
                'canvas.getObjects().filter(o => o.type === "circle" && o.is_background === true).length > 0'
            )
            ...
            final_circles_count = await page.evaluate(
                'canvas.getObjects().filter(o => o.type === "circle" && o.is_background).length'
            )
